...
$mailersend = new MailerSend(['api_key' => 'eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiJ9..Nr2ZcwmziDLNOnBnZSEv8FVpHVK0apfeFkPGiS9QAJgNo0lpOJu8mUAgVULzYbcYx7OYqOaLsfEb-2MoQ3CgwKpa3qFkxOPQUu5qn1UgfmvwYpe7sLeYJgGMsRuqtvRmqQFO5Aq2B8ar0PzVfa010lnEkZrD_9mNQKogC-mGUOmBJNhHjB3X9uTy7iF2kntWyEynQgS0vK2MYrkbmDglcpG6j3Jfu_b0zR2tvi4K9MjywrJ6XBShJqcymZaq3WagM-7crbyVH_6Rh9spgEVWLDeexj1qXJeUrPUidkhDiCoosuWjq9R0NKhZnd3eKyvOE8dYSNjkt316B2-UcyGqED93zYZJdOWpHlciEfP5qoOzIP8SH-XyCmXINfJ6mPcqoUev4P6i6DDYzN5wBoZ6bZDynl8qZ9u9Q4R33lyOP0TddLw2KFrHULNJHhxAhT078pPMX76vebpz56Qbtvdoaw-VNMe5LeQeEGrHuZ3BvE489p8cjDxhMlVOX1klJwSk9ChU01maPhvofXpxyf1N2nAxAVXp-J89E1BbKkMkQdT8gqf9vbrubbugI696plhQDXId0HBfKh5LP4ToBembMzKrB6WY-nvYQXoEtGfVU3vMzK8U0z5jLjtNHKiwMeDZBgA7ib75UyG4RlMmshO28lDKEWZuSstuKe7vuUnTx6U']);
	...
	$company_id = $_GET['id'];
	if ($company_id=="ALL") {
                $sql="select * from nua_company where id in (select company_id from nua_company_invoice) order by id";
		 if ($display=='E') {
                     $sql="select * from nua_company where id in (select company_id from nua_company_invoice) order by id";
                 }
	} else {
	...
	if ($count<11||$grand_total<20000) {
                if ($bbb['org_id']!=17) { 
		$invite_total_float=$grand_total;
                } else {
                $invite_total_float=0;
                }
		$sql="update nua_company_invoice set invite_total_float = " . $invite_total_float . " where id = " . $invoice_id;
                $X->execute($sql);
...
//	    $pdf->Output('F','/var/www/html/d/' . $file_name);
		 ...
		 $a= new Attachment(file_get_contents('/var/www/html/d/' . $file_name), $file_name);
                 ...
                 $emailParams = (new EmailParams())
                    ...
                    ->setTemplateId('vywj2lpnd6mg7oqz')
                    ->setSubject("Per Sri's Request - " . $company['company_name'])
